...
export async function generateChatResponse(message: string): Promise<string> {
  const response = await anthropic.messages.create({
    ...
    messages: [
      {
        role: 'user',
        content: `You are WingMan, a helpful and enthusiastic dating assistant. Help the user plan creative, memorable dates based on their request. Be concise, specific, and practical. Include venue suggestions, activities, and tips.

      Do not return in markdown format.
      Keep it casual with emojis every now and then, do not be overwhelming or pushy.
